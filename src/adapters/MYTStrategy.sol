...
/**
 * @title MYTStrategy
 * @notice The MYT is a Morpho V2 Vault, and each strategy is just a vault adapter which interfaces with a third party
 * protocol
...
contract MYTStrategy is IMYTStrategy, Ownable {
        ...
        returns (bytes32[] memory strategyIds, int256 change)
    {
        ...
        if (adapterParams.action == ActionType.direct) {
            ...
            amountDeallocated =
                _deallocate(assets, adapterParams.swapParams.txData, adapterParams.swapParams.minIntermediateOut);
        } else {
            revert ActionNotSupported();
        }
    ...
    function claimRewards(address token, bytes memory quote, uint256 minAmountOut)
        public
        virtual
        onlyOwner
        returns (uint256)
    ...
    /// @notice this function should be used to estimate the correct amount that can be fully withdrawn, accounting for
    /// losses due to slippage, protocol fees, and rounding differences
