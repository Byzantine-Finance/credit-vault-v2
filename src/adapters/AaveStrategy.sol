...
contract AaveStrategy is MYTStrategy {
    ...
    function _deallocate(uint256 amount) internal virtual override returns (uint256) {
        ...
        if (idleBalance < amount) {
            ...
            if (balanceAfter < balanceBefore + shortfall) {
                revert InsufficientBalance(balanceBefore + shortfall, balanceAfter);
            }
        }
        ...
        returns (uint256)
    {
        ...
        uint256 assetsReceived =
            dexSwap(address(MYT.asset()), address(rewardToken), rewardReceived, minAmountOut, quote);
    ...
    function adminDexSwap(address to, address from, uint256 amount, uint256 minAmountOut, bytes calldata callData)
        external
        onlyOwner
        returns (uint256)
    {
